...
use ui_test::{bless_output_files, error_on_output_conflict, run_tests_generic, Args, CommandBuilder, Config, Match};
...
fn cargo_config(mode: Mode, test_dir: &str, cx: &TestCx) -> Config {
    ...
    config.program.envs.push((
        "RUST".into(),
        Some(cx.driver_path.clone().into_os_string()),
    ));
    config.program.envs.push((
...
fn common_settings(mut config: Config, mode: Mode, bless: bool) -> Config {
    ...
    config.bless_command = Some("./plz test --bless".into());
    ...
    config.out_dir = PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("rp_ui_tests");
    config
}
